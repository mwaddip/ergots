...
- **`opts.epochLength` / `opts.useLastEpochs`** (0.4.0): resolved through the same defaulting/validation as `compareProofs`'s third argument and `PoPowParams` — see [Difficulty functions](#difficulty-functions-040) below, and `type DifficultyParams`. Resolved *before* any other proof inspection, as `verifyParsedProof`'s first step; an invalid value throws `RangeError`, not `ProofVerificationError` — but for this bytes entry point, only after `parseProof` succeeds, since `verifyProof` parses first and only then delegates to `verifyParsedProof`. Malformed bytes with invalid `opts` throw `ProofVerificationError('parse-failed')` instead of `RangeError`.
