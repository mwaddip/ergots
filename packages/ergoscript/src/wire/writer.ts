 ...
 * than imported because cross-package use must go through published
 * package names and the proof package does not export its writer publicly.
 *
 * Deliberate divergence from the proof package: VLQ encoding is exposed as
 * methods on the writer (rather than free functions returning a Uint8Array
 * that gets passed to writeBytes), matching the corresponding choice on
 * `ByteReader` in this package. The wire format and the cursor are tightly
 * coupled — every serializer primitive emits through the same stateful
 * builder.
 *
 ...
 * is intentional and matches the proof package — readers operate on
 * untrusted bytes and need a typed error taxonomy; writers operate on
 * serializer-controlled inputs and any failure is a contract violation.
 */
