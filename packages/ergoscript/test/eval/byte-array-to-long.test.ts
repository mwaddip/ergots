/**
 * ByteArrayToLong arm — fixture-driven evaluation tests.
 *
 ...
 *   if input.len() < 8 { return Err(UnexpectedValue("...at least 8 elements")); }
 *   Ok(((input[0] as i64) << 56 | ... | (input[7] as i64)).into())
 *
 * The `eval_skip_tail` test at byte_array_to_long.rs:62-65 confirms trailing
 * bytes after the first 8 are IGNORED (length comparison is `< 8`, NOT `!= 8`).
 *
 * Cost-charging order: Pattern A — envelope BEFORE eval-child. Fixed(16).
 *
 * Non-Coll[Byte] input is rejected by `ByteArrayToLong::try_build` in
 * sigma-rust at build time (`ergotree-ir/src/mir/byte_array_to_long.rs:41-47`
 ...
 * directly with a hand-built MIR node (calc_blake2b256 precedent).
 */
...
import type { ByteArrayToLong as ByteArrayToLongExpr } from '../../src/mir/types'
import { captureEvalError, hexToBytes, hydrateSValue } from '../_helpers'

const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)
const fixturePath = path.join(__dirname, '../fixtures/eval/byte-array-to-long.json')
...
describe('ByteArrayToLong arm — fixture-driven', () => {
