...
| **2g — Sigma protocol** | `proveDlog` → `proveDhTuple` → `CAND` / `COR` / `CTHRESHOLD` composition. `@noble/curves` added. | Delivered as two sub-slices: **2g-medium** (leaf-only verifier; structural `SigmaBoolean`; `CreateProveDlog` / `CreateProveDhTuple` eval arms; `@noble/curves@2.2.0` adapter; `verifySignature` public function. ✅ 2g-medium shipped 2026-05-16) + **2g-combinators** (the 3 deferred sigma combinators — `Atleast`/`SigmaAnd`/`SigmaOr` eval arms + conjecture verifier extension: `Cand`/`Cor` XOR challenge derivation and `Cthreshold` GF(2^192) polynomial Lagrange interpolation; not yet shipped). |
| **2g.5 — Method-call dispatch** | MethodCall-routed Coll methods (`.indices`, `.zip`, `.zipWith`, `.reverse`, `.flatten`, `.getOrElse`) and numeric shift ops (`SNumericTypeMethods.shiftLeft/Right`). | Method-call-routed operations evaluate against sigma-rust oracle fixtures; the `MethodCall` MIR arm dispatches to per-method handlers. |
...
**Note on realized vs planned phase numbering:** The umbrella plan above uses the original numbered sequence (2a–2j). Implementation revealed that several umbrella phases were each delivered across multiple narrower slices. The realized phase labels used in `facts/ergoscript.md` are: 2f-narrow (Box wire), 2f Stop α/β/γ (Box-extract arms), 2f-medium (chain-state Context), 2f Coll HOFs (9 collection HOF arms), 2g-medium (sigma protocol, leaf-only verifier). The umbrella "2g" row delivered its first sub-slice as 2g-medium (shipped 2026-05-16); the remainder ships as 2g-combinators (sigma combinators + conjecture verifier extension). These realized labels are the historical record in `facts/ergoscript.md`; the umbrella table above maps them to the original planned phases. Do not renumber older docs — the realized labels in `facts/` are stable references.
