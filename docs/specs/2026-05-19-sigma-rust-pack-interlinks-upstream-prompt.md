...
> ✅ **SUPERSEDED 2026-05-19**: landed as [ergoplatform/sigma-rust#866](https://github.com/ergoplatform/sigma-rust/pull/866); cherry-picked to `integration/ergots`. The local fixture-gen workaround module `interlinks_jvm.rs` was removed; all call sites reverted to `NipopowAlgos::pack_interlinks` / `PoPowHeader::check_interlinks_proof` / `NipopowProof::is_better_than`. Fixtures regenerated byte-identical to pre-revert (verified). This file is preserved for historical context — the empirical mainnet validation and analysis below were what motivated the upstream fix.
