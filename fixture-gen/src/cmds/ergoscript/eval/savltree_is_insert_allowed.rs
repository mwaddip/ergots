//! SAvlTree.isInsertAllowed handler — fixtures.
//!
//! Sigma-rust ref: `ergotree-interpreter/src/eval/savltree.rs:59-63`
//! (IS_INSERT_ALLOWED_EVAL_FN)
//! Method registration:
//! `ergotree-ir/src/types/savltree.rs::IS_INSERT_ALLOWED_METHOD`
//!
//! Pattern A cost 15 (charged before obj extraction). Returns `Boolean` —
//! `tree_flags.insert_allowed()` (bit 0 of the packed flag byte).
//!
...
fn make_entry(
    ...
    let expr: Expr = PropertyCall::new(avl_expr, IS_INSERT_ALLOWED_METHOD.clone())
        .unwrap()
        .into();
...
pub fn generate() -> anyhow::Result<EvalFixtureFile> {
    ...
    //  - all flags off → false
    //  - insert-only on (0x01) → true
    //  - insert off but update+remove on (0x06) → false
    //  - all flags on (0x07) → true
    entries.push(make_entry(
        "is_insert_allowed_no_ops",
        digest_pattern(0x00),
    ...
    entries.push(make_entry(
        "is_insert_allowed_update_remove_only",
        digest_pattern(0xAB),
        AvlTreeFlags::new(false, true, true),
        8,
        None,
    )?);
    entries.push(make_entry(
        "is_insert_allowed_all_allowed",
        digest_pattern(0xFF),
    ...
    Ok(EvalFixtureFile {
        corpus: "eval_savltree_is_insert_allowed",
        entries,
    })
}
