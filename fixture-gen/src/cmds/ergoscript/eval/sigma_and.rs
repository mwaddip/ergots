//! SigmaAnd eval arm fixture (phase 2g-combinators Task 5).
//!
//! SigmaAnd is the AND sigma combinator: `SigmaAnd { items: SigmaConjectureItems<Expr> }` —
//! each item is an Expr that evaluates to SSigmaProp, then combined via `candNormalized`.
//!
...
//!   4. candNormalized(items) — TrivialProp(true) absorbed, TrivialProp(false) absorbing
//!
//! Coverage:
//!   - 2-leaf ProveDlogs → Cand([P,Q])
//!   - 3-leaf ProveDlogs → Cand([P,Q,R])
//!   - 5-leaf ProveDlogs → Cand([P,Q,R,S,T])
//!   - TrivialProp(true) child absorbed → Cand([P,Q]) (identity element)
//!   - TrivialProp(false) child → TrivialProp(false) (absorbing element)
//!   - single child after normalization → unwrapped ProveDlog (Cand([X]) → X)
//!   - empty after normalization → TrivialProp(true) (all-true → true)
...
/// Build a SigmaAnd ErgoTree from sigma-boolean items and return (tree, hex).
fn build_tree(items: Vec<SigmaBoolean>) -> anyhow::Result<(ErgoTree, String)> {
    ...
    let body: Expr = SigmaAnd::new(exprs)?.into();
...
#[derive(Serialize)]
pub struct SigmaAndFixture {
    pub description: &'static str,
    pub entries: Vec<SigmaAndEntry>,
}

fn success_entry(name: &str, items: Vec<SigmaBoolean>) -> anyhow::Result<SigmaAndEntry> {
...
pub fn generate() -> anyhow::Result<SigmaAndFixture> {
    ...
    // 2-leaf basic: Cand([P,Q])
    entries.push(success_entry(
        "sigma_and_2_leaf",
        vec![p.clone(), q.clone()],
    )?);

    // 3-leaf basic: Cand([P,Q,R])
    entries.push(success_entry(
        "sigma_and_3_leaf",
        vec![p.clone(), q.clone(), r.clone()],
    )?);

    // 5-leaf basic: Cand([P,Q,R,S,T])
    entries.push(success_entry(
        "sigma_and_5_leaf",
        vec![p.clone(), q.clone(), r.clone(), s.clone(), t.clone()],
    )?);

    // TrivialProp(true) absorbed (identity): [T,P,Q] → Cand([P,Q])
    entries.push(success_entry(
        "sigma_and_true_absorbed",
        vec![trivial_true(), p.clone(), q.clone()],
    )?);

    // TrivialProp(false) absorbing: [F,P,Q] → TrivialProp(false)
    entries.push(success_entry(
        "sigma_and_false_absorbing",
        vec![trivial_false(), p.clone(), q.clone()],
    )?);

    // Single real child after normalization: [T,P] → P (unwrapped)
    entries.push(success_entry(
        "sigma_and_single_after_filter",
        vec![trivial_true(), p.clone()],
    )?);

    // Empty after normalization: [T,T] → TrivialProp(true)
    entries.push(success_entry(
        "sigma_and_empty_after_filter",
        vec![trivial_true(), trivial_true()],
    )?);

    // Mixed ProveDlog + DhTuple
    entries.push(success_entry(
        "sigma_and_mixed_dlog_dhtuple",
    ...
    entries.push(cost_limit_entry(
        "sigma_and_cost_limit_exceeded",
        vec![p.clone(), q.clone()],
        1,
    )?);

    Ok(SigmaAndFixture {
        description: "SigmaAnd eval arm (phase 2g-combinators Task 5). Pattern A cost: addPerItemCost(10, 2, 1, n) BEFORE eval-children. Source: ergotree-interpreter/src/eval/sigma_and.rs.",
        entries,
    })
}
