//! SAvlTree.isUpdateAllowed handler — fixtures.
//!
//! Sigma-rust ref: `ergotree-interpreter/src/eval/savltree.rs:65-69`
//! (IS_UPDATE_ALLOWED_EVAL_FN)
//! Method registration:
//! `ergotree-ir/src/types/savltree.rs::IS_UPDATE_ALLOWED_METHOD`
//!
//! Pattern A cost 15 (charged before obj extraction). Returns `Boolean` —
//! `tree_flags.update_allowed()` (bit 1 of the packed flag byte).
//!
...
fn make_entry(
    ...
    let expr: Expr = PropertyCall::new(avl_expr, IS_UPDATE_ALLOWED_METHOD.clone())
        .unwrap()
        .into();
...
pub fn generate() -> anyhow::Result<EvalFixtureFile> {
    ...
    //  - all flags off → false
    //  - insert+remove on, update off (0x05) → false
    //  - update-only on (0x02) → true
    //  - all flags on (0x07) → true
    entries.push(make_entry(
        "is_update_allowed_no_ops",
        digest_pattern(0x00),
    ...
    entries.push(make_entry(
        "is_update_allowed_insert_remove_only",
        digest_pattern(0x11),
    ...
    entries.push(make_entry(
        "is_update_allowed_update_only",
        digest_pattern(0xAB),
        AvlTreeFlags::new(false, true, false),
        8,
        None,
    )?);
    entries.push(make_entry(
        "is_update_allowed_all_allowed",
        digest_pattern(0xFF),
    ...
    Ok(EvalFixtureFile {
        corpus: "eval_savltree_is_update_allowed",
        entries,
    })
}
