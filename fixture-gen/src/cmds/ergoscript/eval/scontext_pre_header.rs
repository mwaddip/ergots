...
//!
//! Pattern A cost 15 (charged before obj check). Returns wrapped PreHeader
//! from ctx.pre_header.
//!
...
pub fn generate() -> anyhow::Result<EvalFixtureFile> {
    ...
    // Single case: PropertyCall(Context, preHeader) wrapped in a v0 ErgoTree.
    // Expected value: PreHeader wrapped as { kind: 'PreHeader', value: {...} }.
    // Expected cost: 4 (dispatcher) + 1 (Context arm) + 15 (handler) = 20.
    ...
    // Emit opts_json with the preHeader needed for the TS side to build makeContext.
    // The inner shape matches preheader_to_json (coordination contract with
    // rehydrateEvalOpts in test/_helpers/index.ts).
    let opts_json = json!({
