//! ExtractAmount arm — fixtures for `Expr::ExtractAmount(...)` evaluation.
//!
//! Sigma-rust ref: `ergotree-interpreter/src/eval/extract_amount.rs:9-25`
//!   ctx.add_jit_cost(8)?;                            // BEFORE eval-child
//!   let input_v = self.input.eval(env, ctx)?;
//!   match input_v { Value::CBox(b) => Value::Long(b.value.as_i64()), ... }
//!
//! Cost ordering: Fixed(8) charged BEFORE eval-child (Pattern A — envelope-first).
//! Source: sigma-rust eval/extract_amount.rs:15.
//!
//! Coverage:
//!   - Box with value=BoxValue::MIN_RAW (10_800 nanoErg; on-chain minimum).
//!   - Box with value=1_000_000_000 (1 ERG in nanoErg).
//!   - Box with value=100_000_000 (100 mERG; mid-range VLQ encoding).
//!   - Box with value=BoxValue::MAX_RAW (i64::MAX; sigma-rust BoxValue's max).
//!   - 1 cost-limit entry (jitCostLimit=1 < Fixed(8)) → `'cost-limit-exceeded'`.
//!
//! Error case (`'extract-input-not-box'`): sigma-rust's `ExtractAmount::try_build`
//! calls `input.check_post_eval_tpe(&SType::SBox)` and rejects non-SBox inputs at
//! construction time — cannot be serialized through the standard path. The TS-side
//! assertion is covered by inline tests in `test/eval/extract-amount.test.ts`.

use ergotree_interpreter::eval::test_util::try_eval_out;
use ergotree_ir::chain::context::Context;
use ergotree_ir::chain::ergo_box::{ErgoBox, ErgoBoxCandidate, NonMandatoryRegisters};
use ergotree_ir::chain::ergo_box::box_value::BoxValue;
use ergotree_ir::chain::tx_id::TxId;
use ergotree_ir::ergo_tree::{ErgoTree, ErgoTreeHeader};
use ergotree_ir::mir::constant::Constant;
use ergotree_ir::mir::extract_amount::ExtractAmount;
use ergotree_ir::mir::expr::Expr;
use ergotree_ir::serialization::SigmaSerializable;
use serde::Serialize;
use serde_json::{json, Value as JsonValue};
use sigma_test_util::force_any_val;

use super::common::value_to_json;

#[derive(Serialize)]
pub struct ExtractAmountFixture {
    pub name: String,
    pub tree_bytes_hex: String,
    pub opts_json: JsonValue,
    /// null for error entries
    pub expected_value_json: JsonValue,
    /// 0 for error entries
    pub expected_cost: u64,
    /// null for success entries
    pub expected_error_code: JsonValue,
}

#[derive(Serialize)]
pub struct ExtractAmountFixtureFile {
    pub corpus: &'static str,
    pub entries: Vec<ExtractAmountFixture>,
}

/// Build a minimal ErgoTree (v1, hasSize=true, body = Const(SBoolean true)).
///
/// v1 is required: the TS SBox parser reads the ergoTree header and demands
/// `hasSize=true` (bit 3) to bound the read without a full body parse. All
/// real on-chain boxes use v1+. Mirrors `sbox_roundtrip.rs::minimal_ergo_tree`.
fn minimal_ergo_tree() -> ErgoTree {
    let header = ErgoTreeHeader::v1(false);
    let expr = Expr::Const(true.into());
    ErgoTree::new(header, &expr).expect("minimal ErgoTree")
}

/// Build a 32-byte all-zero TxId.
fn zero_tx_id() -> TxId {
    TxId::zero()
}

/// Construct an ErgoBox with the given nanoErg value, zero tokens, empty registers.
fn simple_box(nanoerg: u64) -> ErgoBox {
    let value = BoxValue::new(nanoerg).expect("BoxValue");
    let tree = minimal_ergo_tree();
    let candidate = ErgoBoxCandidate {
        value,
        ergo_tree: tree,
        tokens: None,
        additional_registers: NonMandatoryRegisters::empty(),
        creation_height: 0,
    };
    ErgoBox::from_box_candidate(&candidate, zero_tx_id(), 0).expect("ErgoBox")
}

/// Build an `ExtractAmount { input: Const(SBox, box) }` ErgoTree and return its hex.
fn build_tree(ergo_box: ErgoBox) -> anyhow::Result<(ErgoTree, String)> {
    let box_const: Constant = ergo_box.into();
    let input: Expr = Expr::Const(box_const);
    let expr: Expr = ExtractAmount {
        input: Box::new(input),
    }
    .into();
    let tree = ErgoTree::new(ErgoTreeHeader::v0(false), &expr)?;
    let hex = hex::encode(tree.sigma_serialize_bytes()?);
    Ok((tree, hex))
}

fn success_entry(name: &str, ergo_box: ErgoBox) -> anyhow::Result<ExtractAmountFixture> {
    let (tree, hex) = build_tree(ergo_box)?;
    let ctx = force_any_val::<Context>();
    let val: ergotree_ir::mir::value::Value<'static> = try_eval_out(&tree.proposition()?, &ctx)?;
    Ok(ExtractAmountFixture {
        name: name.into(),
        tree_bytes_hex: hex,
        opts_json: json!({}),
        expected_value_json: value_to_json(&val),
        expected_cost: ctx.jit_cost_value(),
        expected_error_code: json!(null),
    })
}

/// Cost-limit entry — `jitCostLimit` set below Fixed(8) so `addCost` overshoots.
fn cost_limit_entry(name: &str, ergo_box: ErgoBox, limit: u64) -> anyhow::Result<ExtractAmountFixture> {
    let (_tree, hex) = build_tree(ergo_box)?;
    Ok(ExtractAmountFixture {
        name: name.into(),
        tree_bytes_hex: hex,
        opts_json: json!({ "jitCostLimit": limit }),
        expected_value_json: json!(null),
        expected_cost: 0,
        expected_error_code: json!("cost-limit-exceeded"),
    })
}

pub fn generate() -> anyhow::Result<ExtractAmountFixtureFile> {
    let mut entries = Vec::new();

    // BoxValue::MIN_RAW = 360 * 30 = 10_800 nanoErg (Ergo on-chain minimum).
    // Use MIN_RAW as the "min value" boundary case.
    entries.push(success_entry("extract_amount_value_min_raw", simple_box(BoxValue::MIN_RAW))?);

    // 1_000_000_000 nanoErg = 1 ERG
    entries.push(success_entry("extract_amount_value_1bil", simple_box(1_000_000_000))?);

    // A mid-range value with interesting VLQ encoding.
    entries.push(success_entry("extract_amount_value_100m", simple_box(100_000_000))?);

    // BoxValue::MAX_RAW = i64::MAX = 9_223_372_036_854_775_807
    entries.push(success_entry("extract_amount_value_max", simple_box(BoxValue::MAX_RAW))?);

    // Cost-limit: 1 < Fixed(8) — addCost(8) overshoots immediately.
    // Cost is charged BEFORE eval-child, so the limit fires before Const(SBox)
    // is evaluated. The ctx.addCost(8) throws 'cost-limit-exceeded'.
    entries.push(cost_limit_entry("extract_amount_cost_limit_exceeded", simple_box(1_000_000_000), 1)?);

    Ok(ExtractAmountFixtureFile {
        corpus: "eval_extract_amount",
        entries,
    })
}
