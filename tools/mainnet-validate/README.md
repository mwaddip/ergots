...
5. **RESOLVED during phase 2j-b T7 first loop run** (2026-05-23) — shim per-tx loop ordering: data-input lookup must precede input remove. Discovered when the harness halted at h=204570 with `missing-data-utxo` for box `0017269e...`. `walk-trace` revealed that tx#1 at h=204570 uses the box as BOTH input #0 AND data_input #0; the shim's old order (outputs → inputs → data-inputs) removed the box before the data-input get could find it. Per Ergo consensus, data-inputs and inputs evaluate against the same pre-tx UTXO snapshot — both must exist at tx-eval time. Fix swapped the per-tx loop ordering to outputs → data-inputs → inputs. No design spec; minimal mechanical fix.
