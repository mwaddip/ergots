...
Authoritative algorithmic reference: `~/projects/ergo_avltree_rust/` at pin `568e7c3` (upstream PRs #10/#11/#13 included; see "History" under "Source mapping" for the pin's provenance). Where this file is silent on implementation detail, the Rust source is canonical.
...
- **No throws on verification failures.** `verifyAvlBatch` / `verifyAvlLookup` return `null` on verification failure. Throws indicate programmer errors only — with one engine-level carve-out: a pathologically deep proof spine (tens of thousands of nodes; the threshold is engine- and stack-size-dependent, measured between 1e3 and 1e4 levels under plain Node) overflows the JS call stack in the digest check's recursive `label()` (`proof-decode.ts:297` → `node.ts:199-200`) and escapes as a `RangeError`. That is resource exhaustion, not a verification verdict, and it is deliberately not converted to `null`: `ergo_avltree_rust`'s `label` used to recurse the same way and its process aborted on stack exhaustion too; the crate has SINCE been fixed (commit `b785d0d`, an ancestor of `568e7c3`) to label subtrees iteratively via an explicit heap-allocated stack (`Node::label_subtree`, `batch_node.rs:130-157` @568e7c3), closing its own exposure — this package's `label()` (`node.ts`) has not been made iterative to match. The JVM path — the canonical semantic reference — still shares the exposure directly: scrypto's `BatchAVLVerifier` wraps replay in `scala.util.Try`, which catches `NonFatal` only, and `StackOverflowError` is a `VirtualMachineError` that escapes it. No reference-corroborated node bound exists either way — the JVM script-eval path constructs its verifier with no `maxNumOperations` (`CAvlTreeVerifier.scala:17-23`) — so a TS-side cap could reject a proof the reference accepts, an accept/reject fork worse than the crash it prevents. Callers feeding untrusted proofs can set `config.maxNumOperations` (activates the pre-reconstruction node-count bound) or catch `RangeError` at their own boundary. A caught `RangeError` is **indeterminate** — abort or propagate it; never map it to a rejection verdict, which would reintroduce exactly the accept/reject fork this carve-out exists to prevent. Documented by `verifier-adversarial-recursion.test.ts`.
...
| `UpdateLongBy` | add delta to i64 (a sum overflowing i64 fails the op — JVM `addExact` semantics); result=0 → delete | insert with delta (positive) or fail (negative) |
...
Pinned at `~/projects/ergo_avltree_rust/` commit `568e7c3` (upstream PRs #10/#11/#13 included).

**History.** The fork's `main` was rebased 2026-08-03; pre-Phase-E citations
used pin `191052c` (readable via `backup/pre-rebase-main-20260803`). Phase E
(2026-08-04) re-verified every citation at canonical `568e7c3` — the only pin
in use since.
...
| `authenticated_tree_ops.rs::double_left_rotate` (156-198 @568e7c3) | `doubleLeftRotate` (`rotation.ts`) | 1:1 port; fresh `newInternal` allocations instead of Rc<RefCell> in-place update (labelCache invariant). Its two `TypeError` preconditions (`node.right`, `node.right.left`) are a PROVER-side programmer-error backstop only: both verifier call sites (`delete.ts::rebalanceShrinkLeft`, `modify.ts::rotateRightDescent`) guard both operands ahead of the call, so neither throw is reachable from `verifyAvlBatch` |
| `authenticated_tree_ops.rs::double_right_rotate` (205-240 @568e7c3) | `doubleRightRotate` (`rotation.ts`) | 1:1 port (mirror); same allocation policy; same backstop-only status for its `node.left` / `node.left.right` preconditions (guarded by `delete.ts::rebalanceShrinkRight` and `modify.ts::rotateLeftDescent`) |
| `authenticated_tree_ops.rs::modify_helper` (302-431 @568e7c3) | `modifyHelper` + `handleLeafNode` + `handleLeafMatch` + `handleLeafGap` + `handleInternalNode` + `addNode` + `rebalanceLeftDescent` + `rebalanceRightDescent` + `rotateLeftDescent` + `rotateRightDescent` (`modify.ts`) | Decomposed into 10 helpers; `needsDelete` signal added per two-phase dispatch design; handles Lookup/UnknownModification/Insert/Update/InsertOrUpdate/UpdateLongBy (Remove/RemoveIfExists live in delete.ts). `rotateLeftDescent` / `rotateRightDescent` guard the PROMOTED GRANDCHILD (`newLeftm.right` / `newRightm.left`) and return `proof-malformed` — reachable with a crafted balance byte (`addNode`'s split node has two LEAF children, and the balance the verifier reads comes straight out of the proof). Formerly a deliberate divergence: pre-`568e7c3` the reference panicked there (`Node::balance`, `batch_node.rs:174-180` @568e7c3, reached from `double_*_rotate`'s unguarded `.balance()` call). Fixed upstream (`ergo_avltree_rust` commit `c459191`, prompted by this port's own adversarial-rotation tests): `double_left_rotate` / `double_right_rotate` (`authenticated_tree_ops.rs:156-198` / `205-240` @568e7c3) now `ensure!` the promoted grandchild is internal (`:171-174` / `:215-218` @568e7c3) before calling `.balance()` (`:175` / `:219` @568e7c3) and return `Err` — this guard now mirrors the reference instead of diverging from it |
| `authenticated_tree_ops.rs::add_node` (245-259 @568e7c3) | `addNode` (`modify.ts`) | 1:1 port; splits the leaf-gap into (modifiedOriginal, newLeaf) under a new InternalNode with balance=0 |
...
| (TS-only) | `verifyAvlBatchPartial` (`verify.ts`) | v0.2.0 partial-success variant. Wraps the per-op `BatchAvlVerifier.performOneOperation` loop with mid-loop break + pre-op `digest()` snapshot to surface the AFTER-last-successful-op digest. The snapshot is necessary because sigma-rust poisons `root = null` on per-op failure (line 206 of `batch_avl_verifier.rs` @568e7c3), after which `digest()` returns `None`. Backs `@ergots/ergoscript`'s V3+ `SAvlTree.insert/update` handlers, which honor sigma-rust's break-on-failure-with-state-after-last-success semantics. |
...
and `AVLTree::unpack` (`batch_node.rs:682-715` @568e7c3) for well-formed input — four
...
- `~/projects/ergo_avltree_rust/src/` — Rust reference implementation at pin `568e7c3` (verifier + prover; see "History" in "Source mapping")
