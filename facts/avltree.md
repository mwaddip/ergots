...
- **No throws on verification failures.** `verifyAvlBatch` / `verifyAvlLookup` return `null` on verification failure. Throws indicate programmer errors only — with one engine-level carve-out: a pathologically deep proof spine (tens of thousands of nodes; the threshold is engine- and stack-size-dependent, measured between 1e3 and 1e4 levels under plain Node) overflows the JS call stack in the digest check's recursive `label()` (`proof-decode.ts:297` → `node.ts:196`) and escapes as a `RangeError`. That is resource exhaustion, not a verification verdict, and it is deliberately not converted to `null`: both references share the exposure (Rust's `label` recurses the same way and aborts on stack exhaustion; JVM `scala.util.Try` catches `NonFatal` only, which excludes `StackOverflowError`), and the JVM script-eval path constructs its verifier with no `maxNumOperations` (`CAvlTreeVerifier.scala:17-23`), so no reference-corroborated node bound exists — a TS-side cap could reject a proof the reference accepts, an accept/reject fork worse than the crash it prevents. Callers feeding untrusted proofs can set `config.maxNumOperations` (activates the pre-reconstruction node-count bound) or catch `RangeError` at their own boundary. A caught `RangeError` is **indeterminate** — abort or propagate it; never map it to a rejection verdict, which would reintroduce exactly the accept/reject fork this carve-out exists to prevent. Documented by `verifier-adversarial-recursion.test.ts`.
...
| `authenticated_tree_ops.rs::double_left_rotate` (151-180) | `doubleLeftRotate` (`rotation.ts`) | 1:1 port; fresh `newInternal` allocations instead of Rc<RefCell> in-place update (labelCache invariant). Its two `TypeError` preconditions (`node.right`, `node.right.left`) are a PROVER-side programmer-error backstop only: both verifier call sites (`delete.ts::rebalanceShrinkLeft`, `modify.ts::rotateRightDescent`) guard both operands ahead of the call, so neither throw is reachable from `verifyAvlBatch` |
| `authenticated_tree_ops.rs::double_right_rotate` (187-216) | `doubleRightRotate` (`rotation.ts`) | 1:1 port (mirror); same allocation policy; same backstop-only status for its `node.left` / `node.left.right` preconditions (guarded by `delete.ts::rebalanceShrinkRight` and `modify.ts::rotateLeftDescent`) |
| `authenticated_tree_ops.rs::modify_helper` (278-407) | `modifyHelper` + `handleLeafNode` + `handleLeafMatch` + `handleLeafGap` + `handleInternalNode` + `addNode` + `rebalanceLeftDescent` + `rebalanceRightDescent` + `rotateLeftDescent` + `rotateRightDescent` (`modify.ts`) | Decomposed into 10 helpers; `needsDelete` signal added per two-phase dispatch design; handles Lookup/UnknownModification/Insert/Update/InsertOrUpdate/UpdateLongBy (Remove/RemoveIfExists live in delete.ts). Deliberate divergence: `rotateLeftDescent` / `rotateRightDescent` guard the PROMOTED GRANDCHILD (`newLeftm.right` / `newRightm.left`) and return `proof-malformed`; the reference panics there (`AVLTree::balance` `batch_node.rs:382-384` → `Node::balance` `:129-135`, `panic!("not internal node")`, reached from `double_*_rotate` at `authenticated_tree_ops.rs:159` / `:195`). Reachable with a crafted balance byte: `addNode`'s split node has two LEAF children, and the balance the verifier reads comes straight out of the proof |
...
| `authenticated_tree_ops.rs::delete_helper` (468-659) | `deleteHelper` + `deleteInner` + `tryEasyDeleteRightLeaf` + `tryEasyDeleteLeftLeaf` + `hardDeleteLeftDescent` + `hardDeleteRightDescent` + `rebalanceShrinkLeft` + `rebalanceShrinkRight` (`delete.ts`) | Decomposed into 8 helpers; `saved_node` out-param emulated via `SavedNodeRef` wrapper (`{ node: LeafNode \| null }`); second-pass deletion using `replayComparison`. Deliberate divergence: `rebalanceShrinkLeft` / `rebalanceShrinkRight` guard the PROMOTED GRANDCHILD (`rootRight.left` / `rootLeft.right`) as well as the child, and return `proof-malformed`; the reference panics there (same accessor chain as the `modify_helper` row). Reachable with a bare LABEL token in that slot — exactly what a pre-0.4.0 prover emitted for a double-rotation `Remove` |
