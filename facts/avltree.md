...
- **Why a pre-op snapshot:** `ergo_avltree_rust`'s `BatchAVLVerifier` (and the TS port) poisons `root = null` on op failure; `digest()` then returns `null`. Snapshotting `digest()` before each op is the only way to recover the pre-failure state.
...
Prover support: `BatchAVLProver` and `PersistentBatchAVLProver` are now ported to TS (v0.3.0). The verifier and prover share the same mutation engine (`modify.ts` / `delete.ts`) through the `AvlTreeOpsCallbacks` interface. This package's prover and `removedNodes()` corpora are generated by two preserved fork-worktree emitters — `test/fixtures/prover/EMITTER.rs` and `test/fixtures/removed-nodes/EMITTER.rs` — each run against a detached `ergo_avltree_rust` worktree at pin `568e7c3` (regeneration recipe in each file's header); the monorepo's `fixture-gen/` crate is not involved.
