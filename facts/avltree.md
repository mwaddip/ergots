...
4. **Adversarial rejection** (`verifier-adversarial-rotation.test.ts`): hard-coded crafted proofs, one per double-rotation call site, asserting `verifyAvlBatch` returns `null` **and does not throw**. Enforces the "No throws on verification failures" guarantee at the four sites where the reference implementation panics.
