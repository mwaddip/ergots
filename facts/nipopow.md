...
- **Postcondition (failure):** Throws `ProofParseError` with a structural reason (`empty-proof`, `truncated`, `vlq-overflow`, `oversized`, `unexpected-tag`, `trailing-bytes`). The function does NOT silently produce a partial proof, and rejects any trailing bytes after the encoded suffix_tail (including inside the bounded PoPowHeader header/proof subreaders).
