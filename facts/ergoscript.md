...
| Evaluator surface (`evaluate`, `evaluateWith`, `makeContext`, `EvalError` 46 codes, `SValue` / `SType` / `Expr` discriminated unions [canonical], eval arm coverage 52/~70, 38-entry method-handler registry, `EvalOpts` chain-state fields) | [`facts/ergoscript-eval.md`](./ergoscript-eval.md) |
...
| Evaluator | 52 of ~70 `Expr` arms wired; 38 method-handler registry entries; 46 `EvalError` codes; mainnet C2 corpus `success` ≥ 18 (post-2h-c.1 uplift TBD on next corpus run) |
...
Cross-runtime: 2857 ergoscript + 156 avltree + 307 nipopow + 115 scorex = 3435 tests, passing under both `node` and `jsdom`.
