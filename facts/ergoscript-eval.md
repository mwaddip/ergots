...
- `sValueEquals` recursive structural comparer covering primitives, `GroupElement` (byte-equal), `SigmaProp` (byte-equal on opaque `.raw`) **(superseded F3 2026-06-07: recursive costed SigmaBoolean walk + identity-class ECPoints; see the F3 changelog line)** **(register-path narrowed F5 batch 4 2026-06-10: the identity-class compare applies on VALUE-basis paths only — bare GE + Coll elementwise; box-register compares are byte-basis per JVM id-equality; see the F5 batch 4 changelog entry)**, `Coll`, `Tuple`, `Option`. Cross-kind comparison returns `false` (no coercion) — **later version-gated (2026-06-01): at `ctx.treeVersion < 3`, mismatched-NUMERIC `Eq`/`NEq` operands are coerced to the wider kind before comparison (JVM pre-V3 auto-upcast); see the `'bin-op-kind-mismatch'` taxonomy entry.** `Box` / `AvlTree` throw `'not-implemented-yet'` **(2c snapshot — superseded: structural equality via `boxEqual`/`avlTreeEqual`/`preHeaderEqual`/`headerEqual` landed phase 2e/2h)**. Cost charged per sigma-rust's `data_value_comparer.rs` constants.
...
- 17 new method handlers wired (21 → 38 registry entries):
  ...
  - **2 `SContext.*` additions** (Pattern A Fixed(15) each): `headers` (101:2) returns `Coll[Header]` from `ctx.headers`; `lastBlockUtxoRootHash` (101:9) synthesizes `AvlTree(digest=ctx.headers[0].stateRoot, treeFlags=0b111, keyLength=32, valueLengthOpt=null)` **(superseded F5 batch 2 2026-06-08: 101:9 now reads the independent `ctx.lastBlockUtxoRootHash` field — JVM `ErgoLikeContext.lastBlockUtxoRoot` — instead of deriving from headers; see registry row 38)**. Source: `eval/scontext.rs:58-70` and `:83-99`.
...
- V2-header semantic detail: `powOnetimePk` returns 33 zero bytes (identity-point encoding per `EcPoint::default()` → `scorex_serialize`) **(superseded F5 batch 2 2026-06-08: v2 `powOnetimePk` now returns the secp256k1 GENERATOR — JVM `ErgoHeader.scala:57-58` `wForV2 = dlogGroup.generator`; ergots leads sigma-rust; see registry row 33)**; `powDistance` returns `0n` (BigInt).
- Notable quirk: `SHeader.stateRoot` is declared with `SType::SAvlTree` in sigma-rust `types/sheader.rs:127`, but the eval (`sheader.rs:40-44`) returns `Coll[Byte]` (33 bytes). We match the eval, not the type-system declaration **(superseded F5 batch 2 2026-06-08: ergots now returns `AvlTree` synthesized via `avlTreeFromDigest` — matching the JVM `CHeader.scala:29` eval AND the SAvlTree declaration; ergots leads sigma-rust's Coll[Byte] eval quirk; see registry row 26)**.
...
- 2 new method handlers wired (42 → 44 registry entries) — closes the two Tier-3 long-tail deferrals from the 2g.6 demand survey:
  - **`SGroupElement.getEncoded` (7:2)** — Pattern A Fixed(250). Returns 33-byte SEC1-compressed point as `Coll[Byte]`. Reuses existing `bytesToCollByteSValue` helper (no new dependency on `@noble/curves` — the bytes are already SEC1-encoded on `SValue.GroupElement.value`). V0+. Source: `eval/sgroup_elem.rs:15-26`. **(F5 batch 4: output canonical by the GE canonical-bytes invariant — identity is always 33 zeros, matching JVM point re-serialization.)**
...
- Two documented TS-from-sigma-rust divergences (both inherited, neither introduced by this slice):
  - **`DecodePoint` identity**: existing `decodePoint` adapter at `crypto/secp256k1.ts:65-77` checks `isZero33(bytes)` (all 33 bytes zero), while sigma-rust dispatches on `buf[0] !== 0` only. Pre-existing across the verifier surface; not introduced by 2i-a. In-corpus fixtures always produce identity as exactly 33 zero bytes (canonical sigma-rust serialization). Pathological inputs like `[0x00, nonzero, …]` would diverge. **Resolved as deliberate strict-reject in phase 2i-d** — documented centrally at `packages/ergoscript/src/crypto/secp256k1.ts:decodePoint`. Production-unreachable; strict-reject chosen as a safety margin against hand-crafted/hostile inputs. **(SUPERSEDED by iter-24, walker campaign — mainnet h=1,111,884 falsified "production-unreachable"; `decodePoint` now mirrors sigma-rust/JVM exactly: ANY 0x00-lead ⇒ identity, bytes 1..32 never inspected, trailing bytes tolerated, <33 throws — see the current docstring at `crypto/secp256k1.ts:decodePoint`. F5 batch 4 extends this into the GE canonical-bytes invariant across ALL value ingresses; see Type invariants.)**
...
- Two pre-existing TS-from-sigma-rust divergences acknowledged (neither introduced by 2i-b):
  - **`DecodePoint` identity convention** — inherited from 2i-a. Affects `MultiplyGroup` and `Exponentiate` base decode (both use `decodePoint`). Pre-existing across verifier surface. **Resolved in phase 2i-d** — see the 2i-a section above (and its iter-24 supersession note: the adapter is now lenient, 0x00-lead ⇒ identity) and the central docstring at `crypto/secp256k1.ts:decodePoint` for the full rationale.
...
- **Dual-table sync invariant:** the signature catalog (`mir/method-signatures.ts`) and the method-handler registry (`eval/method-call.ts`) share the `(typeId, methodId)` namespace. A handler MAY exist without a signature (eval-only; the call's static type stays `SAny`), but every signature MUST agree with its handler's runtime element type (the static `tRange` equals the `elem`/shape the handler constructs). First generic-output method satisfying it: `SColl.patch` (12:19) — static `Coll[IV]` resolves to `Coll[receiver-elem]`, matching the handler's `{ elem: obj.elem }` (`eval/method-call.ts`). Mechanical enforcement is future work. **Audit V6-SIGNATURE-01 (2026-06-14):** `SAvlTree.insertOrUpdate` (100:16 → `Option[AvlTree]`) and `SHeader.checkPow` (104:16 → `SBoolean`) were eval-only (handler present, signature absent → `exprTpe` = `SAny`); both now carry closed-`tRange` signatures, dual-table-synced to their handler returns (`some`/`noneAvlTree` elem `SAvlTree`; `evalSHeaderCheckPow` Boolean). Primarily ergots-internal type fidelity, but **NOT strictly consensus-neutral**: replacing `SAny` with the concrete return type lets static passes engage where they previously skipped an `SAny` operand (`reference_sany_type_checks_skip_not_fail`) — e.g. `Eq(checkPow(…), Int)` at V3 now rejects (`'bin-op-kind-mismatch'`, `validateBinOpTypes`) where it formerly over-accepted, matching the JVM `check2(SameType)`. A faithfulness GAIN (closes an adversarial over-acceptance), not a fork — no honest tree affected (full conformance corpus + suite green). Test: `test/mir/method-signatures-v6-handlers.test.ts`.
...
- **New pre-eval pass `validateV6Types(tree, body, treeVersion)`** — wired into `dispatchTreeBody` (`eval/evaluate.ts`) beside `validateBinOpTypes`, BEFORE `tryTrivialReduce` / `evalExpr` and before any cost. Two surfaces walked:
  ...
  **Critical faithfulness rule — inspect serialized annotations, NOT computed `exprTpe`.** A v5 lambda's computed type is `SFunc` (synthesized by `exprTpe` of a `FuncValue`), but no `SFunc` type code is serialized for a first-order v5 lambda. Checking computed types would false-reject every valid v5 `map`/`fold` tree. The pass reads only the `.tpe` / `elemTpe` / `explicitTypeArgs` fields that came from `parseSType` on the wire. Annotation-carrying nodes (enumerated exhaustively, tsc-guided): `Const.tpe`, `ConstPlaceholder.tpe`, `Collection.elemTpe` (Exprs arm only; BoolConstants has none), `Upcast.tpe`, `Downcast.tpe`, `GetVar.tpe` (as `varTpe`), `ExtractRegisterAs.tpe` (as `elemTpe`), `DeserializeContext.tpe`, `DeserializeRegister.tpe`, `FuncValue.args[].tpe`, `MethodCall.explicitTypeArgs`, `PropertyCall.explicitTypeArgs` (**audit V6-PROPERTY-TYPEARG-GATE-01, 2026-06-14** — added; `Global.none[T]` 106:10 is the first PropertyCall carrying a wire type tail, and `childrenOf` walks only the receiver `obj`, so the tail needs its own enumeration; without it a pre-V3 dead-branch `none[UnsignedBigInt]` over-accepted where the JVM rejects at type deserialization. **Residual:** this closes the *type* leg only — the broader v6-**method**-version gate is eval-time (`method-call.ts` `minVersion` → `'tree-version-too-low'`) and is bypassed in a dead branch for *all* v6 methods, so e.g. a pre-V3 dead-branch `none[SInt]` still over-accepts where the JVM rejects via the method-version gate, independent of the type arg; closing it needs a whole-tree `validateMethodVersions` pre-pass — candidate future work, on the v6 PR residuals list). **`ValUse.tpe` is deliberately excluded**: it is computed from the enclosing `ValDef`'s RHS type at parse time (not deserialized from a type-code byte), so a higher-order `val f = <lambda>; … f …` would give `ValUse.tpe = SFunc` on a valid v5 tree and checking it would false-reject.
...
- **F5 batch 4 (2026-06-10):** equality bases + GE canonical-bytes invariant + atLeast children cap.
  - **Equality bases (the JVM verdict basis per compared kind):** bare `GroupElement` and
    ...
    input bytes (ErgoBox.scala:94-97), so `boxEqual` is a pure id compare (blake2b over retained-or-
    canonical bytes, via `boxIdOf`/`boxBytesOf`): two boxes differing only in garbage-vs-canonical
    identity GE register encodings are UNEQUAL (both decode to the identity point; the ids still differ). `Header` Eq/NEq = **id
    basis** — `CHeader.equals` compares ids = blake2b over the CACHED INPUT bytes
    (ErgoHeader.scala:133-140 id-over-`_bytes`; :167-180 parse caches the consumed slice) — the
    13-field walk in `headerEqual` (incl. `id`) is verdict-equivalent. `PreHeader` = field basis in
    the JVM (`CPreHeader` case class, point-equality minerPk) but adversarially unreachable (no
    SPreHeader DataSerializer arm → no `deserializeTo[PreHeader]`; one preHeader per context) —
    byte-field compare kept as-is, document-only. Flat per-type costs match
    DataValueComparer.scala:44-71: EQ_GroupElement 172 / EQ_Box 6 / EQ_PreHeader 4 / EQ_Header 6.
  - **GE canonical-bytes invariant** — see the Type-invariants bullet. Egress (`getEncoded` 7:2,
    `Global.serialize`, equality) emits/compares canonical bytes by construction; matches JVM point
    re-serialization (GroupElementSerializer.scala:20-33).
  - **atLeast 255-children cap** — `'atleast-too-many-children'` (taxonomy, 2g-combinators section).
    JVM order = per-item charge → cap throw → degenerate reductions (`AtLeast.eval` trees.scala:314-320
...
- **Coverage caveat:** 68 of 68 implementable `Expr` variants have implemented arms (F5 batch 4 added the 68th — `LastBlockUtxoRootHash`, the bare `0xa6` op-form). 21 wire opcodes (ModQ family, OpTrue/False, UnitConstant, Select1-5, CollShift/Rotate families, SomeValue, NoneValue, plus FlatMap/TrivialPropFalse/TrivialPropTrue — reclassified from `'not-implemented-yet'` in F5 batch-6) are reserved in sigma-rust's `OpCode` enum but unconditionally parse-rejected — the JVM rejects each identically via `CheckValidOpCode` (rule 1002, `getSerializer` returns null). We mirror via `ExprParseError 'opcode-reserved'`. (`FunDef` (`0xd7`) was the 19th — v6 P6 now parses it as a `ValDef` carrying `tpeArgs` and evaluates it via the `ValDef` arm; see the P6 changelog above and [`facts/ergoscript-wire.md`](./ergoscript-wire.md).) The non-bare forms of those last three reach us through other paths (flatMap as an SColl method-call; the TrivialProp pair as a SigmaBoolean leaf inside a `SigmaPropConstant`, parsed by `wire/sigma-boolean.ts` — a wholly separate path), so the wire layer no longer emits `'not-implemented-yet'`. Trees whose body reaches a not-yet-implemented method-call handler or an eval path with a defensive `EvalError 'not-implemented-yet'` (3 sites: `eval.ts:232`, `global-vars.ts:136`, `bin-op/bit.ts:58`) still throw at runtime. The `evaluate` signature itself is stable; phase 2j adds cost calibration.
...
type SValue =
  ...
  | { kind: 'GroupElement'; value: Uint8Array }   // 33 bytes, CANONICAL SEC1 (F5 batch 4): 33×00 identity | curve-validated 02/03-lead — see invariant bullet below
...
- ~~`'atleast-bound-out-of-range'`~~ — **REMOVED in F1 Task 2 (2026-06-06).** The JVM `Atleast` arm applies degenerate-bound reductions BEFORE any range check: `bound ≤ 0` → `TrivialProp(true)` (SigmaAnd of empty = vacuous truth); `bound > items.length` → `TrivialProp(false)` (unachievable threshold — AFTER the item-count is known at eval, so cost is still charged per-item via Pattern B before the reduction). There is **no eval-time 255-bound cap** — the 255-CHILDREN limit (`MaxChildrenCount`) on the collection length is a separate constraint, enforced as `'atleast-too-many-children'` since F5 batch 4 (next bullet). Cost is unchanged: the per-item `addPerItemCost(20, 3, 5, n)` is always charged before the reductions run. Both degenerate reductions land on the side of a live `cthresholdReduce` path and produce a `SigmaProp(TrivialProp)` SValue directly. Source: JVM `CSigmaDslBuilder.atLeast` + sigma-rust `atleast.ts` (ergo-node-integration); pinned by SANTA conformance vectors `atLeast_bound_zero.json` / `atLeast_bound_exceeds_size.json`.
- **`'atleast-too-many-children'`** (F5 batch 4, 2026-06-10) — `Atleast` arm: the evaluated input collection holds MORE than 255 SigmaProps (`MaxChildrenCountForAtLeastOp = 255`, JVM SigmaConstants.scala:65; CTHRESHOLD's GF(2^192) polynomial arithmetic takes single-byte inputs, so >255 children are unrepresentable). Thrown AFTER the Pattern-B per-item charge and BEFORE the degenerate-bound reductions — the JVM order: `AtLeast.eval` (trees.scala:314-320) charges `addSeqCost` then calls `CSigmaDslBuilder.atLeast`, whose cap throw (CSigmaDslBuilder.scala:102-108) precedes `AtLeast.reduce` (the degenerates live INSIDE reduce, trees.scala:340-359). So `atLeast(0, >255 children)` THROWS — it does not reduce to TrueProp. eni applies the cap only in the non-degenerate path (returns TrueProp there) — a JVM↔sigma-rust fork; ergots follows the JVM. Adversarial-only (`ConcreteCollection` parses to u16 length; compilers never emit >255-prop atLeast).
...
### Phase 2f (pre-existing, uncounted until F5 batch-4 close-out, 2026-06-10)

- **`'coll-map-elem-type-infer-failed'`** — `sValueType` (`eval/svalue-type.ts:66`): the `default` arm of the exhaustive `switch (v.kind)` over every `SValue` variant. The `SValue` discriminated union is fully covered by the switch's explicit arms; tsc proves the default unreachable at compile time under strict mode. Thrown only if a NEW `SValue.kind` is added to `mir/types.ts` without a corresponding arm being added to `sValueType` — the throw is the runtime tripwire that surfaces such a gap immediately on the first `sValueType` call for the new kind. Also emitted by the predecessor `inferSType` local function in `eval/coll-map.ts` (same code, same tsc-unreachable posture). Counted per the defensive-code convention (precedent: `'unsigned-bigint-negative'` and `'sigma-prop-is-proven-no-eval'` are documented as defensive/unreachable guards and counted). Code originated in phase 2f alongside the Coll HOFs; first referenced in this file at the v6 P2b changelog (~line 325 in original); never previously included in the tally. Source: `eval/svalue-type.ts:66` (default arm).
...
| 26 | `SHeader.stateRoot` | 104:5 | 10 | A | `AvlTree` synthesized from the 33-byte stateRoot digest (flags `0b111`, keyLength 32, valueLengthOpt None) — JVM `CHeader.scala:29` `CAvlTree(avlTreeFromDigest(...))`; ergots LEADS sigma-rust here (which returns Coll[Byte], `sheader.rs:40-44`) | `:40-44` (JVM canonical) |
...
| 38 | `SContext.lastBlockUtxoRootHash` | 101:9 | 15 | A | `AvlTree` read from the independent `ctx.lastBlockUtxoRootHash` field (JVM `ErgoLikeContext.lastBlockUtxoRoot`); throws `'context-field-missing'` if absent. F5 batch 2: NO LONGER derived from `ctx.headers[0].stateRoot` (that was sigma-rust `scontext.rs:83-99`); the walker now supplies the field as `avlTreeFromDigest(headers[0].stateRoot)`, the conformance dummy ctx supplies `AvlTreeData.dummy` | `ErgoLikeContext` (JVM canonical) |
...
| 126 | `SPreHeader.version` | 105:1 | 10 | A | `Byte` (`obj.value.version`) — no version gate (v1+) | JVM `methods.scala:1841` (SPreHeaderMethods) |
| 127 | `SPreHeader.nBits` | 105:4 | 10 | A | `Long` (`BigInt(obj.value.nBits)`; the struct field is a u32 number, the accessor is typed `SLong`) — no version gate | JVM `methods.scala:1844` |
| 128 | `SPreHeader.votes` | 105:7 | 10 | A | `Coll[Byte]` (3-byte `obj.value.votes` via `bytesToCollByteSValue`) — no version gate | JVM `methods.scala:1847` |
...
**68 of 68 implementable `Expr` variants** have arms (67 post-2i-c; 68th added in F5 batch 4). 21 variants in sigma-rust's `OpCode` enum are reserved-but-never-dispatched and parse-reject via `ExprParseError 'opcode-reserved'` (was 18 — FlatMap/TrivialPropFalse/TrivialPropTrue reclassified from `'not-implemented-yet'` in F5 batch-6, the JVM rejects all three via the same `CheckValidOpCode` path; their non-bare forms reach us elsewhere — flatMap as an SColl method-call, the TrivialProp pair as a SigmaBoolean leaf in a `SigmaPropConstant`; was 19 before that — `FunDef` (`0xd7`) is parsed + evaluated as a `ValDef` from v6 P6). The 68 implementable variants:
- 8 from phase 2b
