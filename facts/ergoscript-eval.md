...
| 118 | `SGlobal.deserializeTo[T]` | 106:4 | `PerItemCost(100, 32, 32)` on input byte length | A | generic `T` (resolved from `explicitTypeArgs['T']` at call site via P0 engine); parses `args[0]` bytes as an SValue of type `T` via the data codec (`DataSerializer.deserialize` — NO ErgoTree body parse, NO `exprTpe` match); trailing bytes ignored; `MaxTreeDepth(110)` enforced data-driven (actual parse-recursion depth via parseSValue `maxDepth`, NOT type nesting — a deep type with empty data is accepted; box-register + whole-tree-Constant depth are documented residuals); guards `obj.kind === 'Global'` AND `args.length === 1`; MethodCall opcode — **V3-gated** (`minVersion: 3`) | JVM `methods.scala:1906` |
