...
| 130 | `SBox.propositionBytes` | 99:2 | 10 | A | `Coll[Byte]` (`box.ergoTreeBytes`) — method-form of `ExtractScriptBytes`; no version gate | JVM `commonBoxMethods` |
| 131 | `SBox.bytes` | 99:3 | 12 | A | `Coll[Byte]` (`boxBytesOf` — retained slice / re-serialization) — method-form of `ExtractBytes`; no version gate | JVM `commonBoxMethods` |
| 132 | `SBox.bytesWithoutRef` | 99:4 | 12 | A | `Coll[Byte]` (`serializeBoxBytesWithoutRef` — canonical) — method-form of `ExtractBytesWithNoRef`; no version gate | JVM `commonBoxMethods` |
