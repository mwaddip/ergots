...
**Carve-out 4 — trailing inside a `hasSize` body (2026-06-17):** the inner-trailing reject was retired (it matched neither reference). A `hasSize` tree whose body parses cleanly but leaves trailing within the declared size now PARSES; the `ParsedErgoTree` re-serializes its actual body WITHOUT the trailing, so `serializeTree(parseTree(b)) ≠ b`. Adversarial-only (an honest tree's declared size equals its body); on the box path the verbatim span is retained on `ergoTreeBytes` (Carve-out 2 mechanism), so boxes still round-trip. **Cursor/boundary note (pre-existing, unchanged here):** for hasSize, `parseTreeFromReader` advances the outer cursor the FULL declared size (a bounded sub-reader — matches sigma-rust and the now-deleted `consumeTreeFromReader`, byte-identical pre/post this change), whereas the JVM is size-unbounded and lands at the parse-determined end. They diverge only on a hand-crafted **SigmaProp-rooted** tree with genuine inner trailing inside a box (different next-field offset → different boxId) — adversarial-only, tracked as the boundary residual in the spec below. The mainnet burn box h=545,684 is NOT a parse-end case: its **non-SigmaProp** root makes the JVM degrade it to `UnparsedErgoTree` capturing the full declared size, so both impls land at the same byte and accept the box (ergots stores a `Parsed` Const, the JVM an `Unparsed` blob, but the `ergoTreeBytes` span is identical). Tracked in `docs/specs/2026-06-17-ergotree-deserialize-unification.md` ("Parsed vs Unparsed" + boundary residuals).
