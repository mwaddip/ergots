...
- **`'sbox-ergo-tree-no-size'`** — removed. `parseSValue(SBox)` handles v0+hasSize=false ErgoTrees via `consumeTreeFromReader` (the body Expr grammar is self-delimiting); the rejection was an incorrect assumption — ~99% of mainnet boxes use v0 P2PK trees without a size prefix.
