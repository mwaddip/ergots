...
the wire `'not-implemented-yet'` site count from 4 to 3 (the remaining 3 — `FlatMap`/`TrivialPropFalse`/`TrivialPropTrue` — were later reclassified to `'opcode-reserved'`, taking the wire `'not-implemented-yet'` count to 0; see the ExprParseError taxonomy above)
